    ...
    def capture(self, attackerStone):
        ...
        for x, y in self.findNeighbours(attackerStone):
            if(self.points[x, y] != None and self.points[x, y].color != attackerStone.color and self.countLiberty(self.points[x, y]) == 0):
                ...
                if(checkko(self.points[x, y], attackerStone)):
                    self.koPosition = (x, y)
            ...
            def BFS(self , x, y):
                ...
                for i, j in q:
                    if(vis[i][j] == 1):
                        continue
                    vis[i][j] = 1
                    if(self.points[x, y] == 'Black'):
                        Bflage = True
                    elif(self.points[x, y] == 'White'):
                        Wflage = True
                    else:
                        ++cnt
                        stoneX.position = (i, j)
